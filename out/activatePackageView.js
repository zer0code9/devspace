...
function activatePackageView() {
    ...
    const packageViewProvider = new PackageViewProvider_1.PackageViewProvider();
    vscode.window.registerTreeDataProvider('devspace.packageView', packageViewProvider);
    /* Package Status */
    const packageStatus = new PackageStatus_1.PackageStatus(vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100));
    /* Package History */
    ...
    vscode.commands.registerCommand('devspace.openPackageFolder', async () => {
        ...
        if (packageRoot === undefined || packageManagerType === "") {
            return;
        }
        ...
        vscode.workspace.updateWorkspaceFolders(vscode.workspace.workspaceFolders ? vscode.workspace.workspaceFolders.length : 0, null, { uri: vscode.Uri.parse(packageRoot) });
    });
    /**
    ...
    vscode.commands.registerCommand('devspace.changePackageManager', () => {
        ...
        packageManagerPick.onDidAccept(async () => {
            let packageManagers = vscode.workspace.getConfiguration('devspace').get('packageManagers');
            if (!packageManagers || packageManagers.length === 0) {
                return;
            }
            if (packageManagerType === "crate") {
    ...
    vscode.commands.registerCommand('devspace.showPackageHistory', () => {
        ...
        let packageSelect = [];
        packageHistory.map((projectRoot) => {
        ...
        packageHistoryPick.onDidAccept(async () => {
            const projectRoot = packageHistoryPick.selectedItems[0].label;
    ...
    vscode.commands.registerCommand('devspace.addPackageItemPrompt', () => {
        ...
        packageManagerTypePick.onDidAccept(async () => {
            ...
            if (packageManagerType === "node") {
                ...
                packageManager = packageManagers[1];
            }
            else if (packageManagerType === "crate") {
                packageManager = packageManagers[2];
            }
     ...
     */
    vscode.commands.registerCommand('devspace.addPackageItem', (packageManagerType, packageManager) => {
        const packageRoot = vscode.workspace.getConfiguration('devspace').get('packageRoot');
     ...
     * Function: Get Terminal Command
     */
    vscode.commands.registerCommand('devspace.updatePackageItem', (pkg) => {
        const terminal = vscode.window.createTerminal({ name: `Devspace Terminal` });
        terminal.show();
     ...
     * Function: Get Terminal Command
     */
    vscode.commands.registerCommand('devspace.removePackageItem', (pkg) => {
        const terminal = vscode.window.createTerminal({ name: `Devspace Terminal` });
        terminal.show();
     ...
     * Command: Open
     */
    vscode.commands.registerCommand('devspace.openPackageItem', (pkg) => {
    ...
    vscode.window.onDidChangeActiveTextEditor(async (editorE) => {
        ...
        if (areSame) {
            ...
            if (folderPath) {
                if (packageHistory.includes(folderPath)) {
                    packageHistory.splice(packageHistory.indexOf(folderPath), 1);
                }
                packageHistory.push(folderPath);
                if (packageHistory.length > 10) {
                    packageHistory.splice(0, 1);
                }
            }
 ...
 */
function getTerminalCommand(action, pkg, dev, packageManagerType = "node") {
    ...
    if (packageManagerType === "node") {
        ...
        const packageManager = packageManagers[1];
 ...
 * Return: string
 */
function getPackageType(packageRoot) {
    if (packageRoot) {
...
//# sourceMappingURL=activatePackageView.js.map